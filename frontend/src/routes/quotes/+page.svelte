...
<div class="px-8 py-8 mx-auto max-w-[1480px]">
    <div class="mb-8 flex items-end justify-between gap-6 flex-wrap">
        <div>
            ...
            <h1 class="font-h2 text-h2 text-on-surface font-semibold">Sales Quotes.</h1>
            <p class="font-body-md text-sm text-secondary mt-2 max-w-2xl">
                Live from Business Central. Download any quote as the BC-rendered PDF.
            </p>
        </div>
        <div class="flex items-end gap-3 flex-wrap">
            <a href="/quotes/new" class="btn-primary">+ New Quote</a>
        </div>
        <form method="GET" class="flex items-end gap-3 flex-wrap">
            <div class="flex flex-col gap-1">
                <label for="sq-q" class="field-label">Search</label>
                <input
                    id="sq-q"
                    name="q"
                    value={data.q}
                    placeholder="Quote #, customer #, or name"
                    class="field w-[280px]"
                />
            </div>
            <div class="flex flex-col gap-1">
                <label for="sq-status" class="field-label">Status</label>
                <select
                    id="sq-status"
                    name="status"
                    class="field w-[140px]"
    ...
    <div class="card overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-zinc-100 flex-wrap gap-3">
            <p class="eyebrow">
                {data.quotes.length.toLocaleString()} quote{data.quotes.length === 1 ? '' : 's'}
                {#if data.quotes.length > 0}
                    <span class="text-on-surface ml-2">· total {fmtMoney(totalValue, data.quotes[0]?.currencyCode || 'CAD')}</span>
                {/if}
        ...
        {#if data.quotes.length === 0}
            <p class="text-center text-secondary py-20 font-body-md text-sm">No quotes match the filter.</p>
        {:else}
                    ...
                    <tbody>
                        {#each sortedQuotes as q}
                            <tr>
                                ...
                                <td>
                                    <div class="font-medium">{q.shipToName || q.customerName || '—'}</div>
                                ...
                                <td class="muted whitespace-nowrap">{fmtDate(q.documentDate)}</td>
                                <td class="muted whitespace-nowrap">{fmtDate(q.validUntilDate)}</td>
